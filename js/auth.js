...
function checkAuth(allowedRoles = []) {
    ...
    const basePath = baseUrl ? baseUrl + '/' : getBasePath();

    if (!token || !userJson) {
        // Clear anything corrupt
        localStorage.removeItem('ll_token');
        localStorage.removeItem('ll_user');
        window.location.href = baseUrl ? baseUrl + '/user/login.php' : (basePath + 'index.php');
        return;
    }
    ...
    if (allowedRoles.length > 0) {
        ...
        if (!isAllowed) {
            window.location.href = baseUrl ? baseUrl + '/home.php' : (basePath + 'home.php');
            return;
        }
    }
...
function logout() {
    ...
    localStorage.removeItem('ll_user');
    const baseUrl = (typeof APP_URL !== 'undefined') ? APP_URL : '';
    window.location.href = baseUrl ? baseUrl + '/user/login.php' : (getBasePath() + 'index.php');
}
...
async function apiFetch(endpoint, options = {}) {
    ...
    const url = baseUrl ? (baseUrl + '/' + cleanEndpoint) : (basePath + cleanEndpoint);
...
// Initialize theme toggle functionality on load
document.addEventListener('DOMContentLoaded', function() {
    const currentTheme = localStorage.getItem('theme') || 'light';
    // Sync both html and body elements
