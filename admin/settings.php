    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px; overflow: hidden; border: none; box-shadow: 0 15px 35px rgba(0,0,0,0.2);">
            <div class="modal-header bg-dark text-white p-3 border-0 d-flex justify-content-between align-items-center">
                <h5 class="modal-title font-heading" id="participantsModalLabel">
                    <i class="bi bi-people-fill text-danger me-2"></i>Registered Participants
                </h5>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // ----------------------------------------------------
    // Load Forecast Demand (CSV analytics API)
    // ----------------------------------------------------
    ...
    invForm.addEventListener('submit', async function(e) {
        ...
        invBtn.setAttribute('disabled', 'true');
    ...
    donForm.addEventListener('submit', async function(e) {
        ...
        donBtn.setAttribute('disabled', 'true');
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                donAlert.innerHTML = `<strong>Success!</strong> Donation logged. Bag ID assigned: <code>${result.data.bag_id}</code>.`;
            ...
            donBtn.removeAttribute('disabled');
        }
    });

    // ----------------------------------------------------
    // Camps Management (Creation & List)
    // ----------------------------------------------------
    ...
    campForm.addEventListener('submit', async function(e) {
        ...
        campBtn.setAttribute('disabled', 'true');
        ...
        try {
            ...
            campBtn.removeAttribute('disabled');
        }
    });
    ...
    window.openParticipantsModal = async function(campId, campName) {
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="4" class="text-center py-4 text-secondary">
                    <div class="spinner-border spinner-border-sm text-danger me-1"></div> Loading participants...
                </td>
            </tr>
        `;
                    ...
                    list.forEach(p => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td>
                                ...
                                <small class="text-secondary">${escapeHtml(p.phone)}</small>
                            </td>
                            <td>
                                <span class="badge bg-danger rounded-pill px-2 py-1">${escapeHtml(p.bloodType)}</span>
                            </td>
                            <td>
    ...
    window.toggleAttendance = async function(checkbox, registrationId) {
        ...
        try {
            const result = await apiFetch('api/camps.php?action=toggle_attendance', {
                method: 'POST',
    ...
    window.deleteCamp = async function(campId) {
        ...
        try {
            const result = await apiFetch(`api/camps.php?id=${campId}`, {
                method: 'DELETE'
            });
    ...
    urgentForm.addEventListener('submit', async function(e) {
        ...
        urgentSubmitBtn.setAttribute('disabled', 'true');
        ...
        try {
            ...
            urgentAlert.textContent = error.message || "Operation failed.";
            urgentAlert.style.display = 'block';
        } finally {
            urgentSpinner.style.display = 'none';
            urgentSubmitBtn.removeAttribute('disabled');
        }
    });
    ...
    window.deleteUrgentRequest = async function(id) {
        ...
        urgentListAlert.style.display = 'none';

        try {
            const result = await apiFetch(`api/api.php?endpoint=urgent_requests&id=${id}`, {
                method: 'DELETE'
            });
