...
/**
 * Scans the inventory CSV once to find the most recent Store_Date present
 * in the dataset. The 24-month analytics window is anchored to this date
 * rather than the system clock, since the CSV is a static historical
 * dataset that doesn't grow automatically — anchoring to "today" would
 * leave the most recent months empty and skew the demand forecast.
 * Falls back to today's date if the file is missing, empty, or unreadable.
 */
function getLatestStoreDate($csvFile) {
    $latest = null;
    ...
    if (file_exists($csvFile) && ($handle = fopen($csvFile, 'r')) !== false) {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $storeDate = trim($row[4]);
            if ($storeDate === '') continue;
            if ($latest === null || $storeDate > $latest) {
                $latest = $storeDate;
            }
        }
        fclose($handle);
    ...
    return $latest !== null ? new DateTime($latest) : new DateTime('now');
}
...
function getAnalyticsData() {
    ...
    // Dynamic 24-month list leading up to the most recent date actually present
    // in the dataset (rather than a hardcoded literal, which would silently
    // drift out of sync with real time). We scan Store_Date once to find it.
    $monthsList = [];
    $currentDate = getLatestStoreDate($csvFile);
